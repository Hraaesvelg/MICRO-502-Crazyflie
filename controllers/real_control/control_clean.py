...
if __name__ == '__main__':
    ...
    def get_grid_point():
        ...
        idx = 0
        return [point_x, point_y], idx

    [px, py], index = get_grid_point()

    ## coordinates of the landing pad (one for first landing and one for second landing ? )
            ...
            for log_entry in logger:
                ...
                if state != 'land':  # time.time()-startTime<7.5:
                    if endTime - time.time() < 2:  # Land state triggered when 2 seconds left
                        ...
                        if abs(landing[0][0] - data['stateEstimate.x']) < 0.1 and abs(landing[0][1] - data[
                            'stateEstimate.y']) < 0.1:  ## BASED ON THE LANDING COORDINATES THAT WE HAVE (COMPUTED LATER IN THE CODE)
                            height_desired -= 0.02
                        if height_desired < 0.05:
                            state = 'land'  # initally 'landing'
                        kp = 0.5
                        cf.commander.send_hover_setpoint(kp * (landing[0][0] - data['stateEstimate.x']),
                                                         kp * (landing[0][1] - data['stateEstimate.y']), 0,
                                                         height_desired)  # PD Control
                    else:
                        speed_max = 0.2
                        print(zone)
                        if zone == 'FINAL' or zone == 'START':
                            speed_max = 0.2
                            print('difference', data['kalman.stateZ'] - height_desired, landing[0][0],
                                  time.time() - startTime)
                            ...
                            if data['kalman.stateZ'] - previous > 0.022 and landing[0][
                                0] == 0 and time.time() - landing_time > 9.:  # SAVE PAD LOCATION  # (data['kalman.stateZ']-height_desired) < -0.1
                                height_desired -= 0.1
                                landing[0][:] = [data['stateEstimate.x'],
                                                 data['stateEstimate.y']]  # keep starting point in memory
                        ...
                        if math.sqrt((px[index] - data['stateEstimate.x']) ** 2 + (
                                py[index] - data['stateEstimate.y']) ** 2) < 0.1:
